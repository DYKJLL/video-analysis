...
name: scrapling
description: |
  49,709 ⭐ 自适应 Web 爬虫框架，支持反机器人绕过（Cloudflare Turnstile）、隐身浏览器、Spider 爬虫框架、自适应解析、JS 动态渲染。
  适用：网站爬取、数据提取、反爬绕过、动态页面抓取、大规模爬虫、长期监控（网站改版自适应）。
  触发：当用户提到爬虫、数据抓取、绕过反爬、网站爬取等场景时使用。
  安装：pip install "scrapling[all]>=0.4.8" && scrapling install --force
version: "0.4.8"
